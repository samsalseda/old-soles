...
class AFRM(tf.keras.layers.Layer):
    def __init__(self, size=[256, 4, 4], splits=4):
        ...
        self.ffwd = tf.keras.Sequential(
            [
                tf.keras.layers.Conv2D(
                    2 * self.num_channels, self.num_channels, padding="valid"
                ),
    ...
    def call(self, x):
        ...
        split_output = tf.reshape(
            tf.concat(tf.split(output, self.num_splits, axis=3)),
            (self.num_splits, -1, 1, self.lstm_size),
        )
        reversed_split_output = tf.reshape(
            tf.reverse(tf.concat(tf.split(output, self.num_splits, axis=3)), axis=3),
            (self.num_splits, -1, 1, self.lstm_size),
        )

        initial_state = (
            tf.zeros((2, batch_size, self.lstm_size)),
            tf.zeros((2, batch_size, self.lstm_size)),
        )
        fwd_output = []

        for i in range(self.num_splite):
            lstm_output, initial_state = self.forward_LSTM(
                split_output[i], initial_state
            )
            lstm_output = tf.reshape(
                lstm_output,
                (-1, self.num_channels, self.height, int(self.width / self.num_splits)),
            )
        ...
        for i in range(self.num_splite):
            lstm_output, initial_state = self.reverse_LSTM(
